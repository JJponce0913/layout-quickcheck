...
def save_bug_report_custom(
    ...
    matched_rule_folder = find_matching_rule_folder(folder_name, rule_name)

    if matched_rule_folder is not None:
        parent_bug_folder = matched_rule_folder
    else:
        parent_bug_folder = os.path.join("bug_reports", folder_name)
        os.makedirs(parent_bug_folder, exist_ok=True)

    bug_folder = file_config.getCustomTimestampBugReport(parent_bug_folder)
    print(f"[save_bug_report_custom] parent_bug_folder: {os.path.abspath(parent_bug_folder)}")
