...
class Counter():
    ...
    def __init__(self, bug_limit=0, test_limit=0, crash_limit=1, feedback_interval=100):
        ...
        self.total_sort_seconds = 0.0       # Cumulative time spent sorting bugs
        self.total_true_minify_seconds = 0.0  # Cumulative time spent in the minimization loop
    ...
    def addSortTime(self, seconds):
        if seconds is None:
            return
        self.total_sort_seconds += max(0.0, float(seconds))

    def addTrueMinifyTime(self, seconds):
        if seconds is None:
            return
        self.total_true_minify_seconds += max(0.0, float(seconds))
    ...
    def getStatusString(self):
        ...
        if self.feedback_triggered or interval_triggered:
            ...
            ret = f"Passed: {self.num_successful};  Bugs: {self.num_error};"
            if self.num_no_mod_styles_bugs > 0:
                ret += f"  Bugs With No Modified Styles: {self.num_no_mod_styles_bugs};"
            if self.num_cant_reproduce > 0:
                ret += f"  Can't Reproduce: {self.num_cant_reproduce};"
            if self.num_crash > 0:
                ret += f"  Crashes: {self.num_crash}"
            return ret
        else:
            return None
